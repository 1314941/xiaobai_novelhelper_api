from web_api import chat_ollama,chat_ollama_stream,chat_ollama_stream_post
...
for file in txt_files:
    ...
    for item in data:
        if item['role'] == "人物卡生成器":
            character_message=chat_ollama_stream_post("小说情节\n"+input_message+"\n旧的人物卡\n"+character_message+"\n生成新的人物卡",item)
...
>>>>>>> 696561b1ac011b64aaadff995c1946f650c037e6
    ...
    for item in data:
        if item['role'] == "细纲生成器":
            if begin:
                message=main_message+character_message+f"生成第{count}章的细纲"
    ...
    for item in data:
        if item['role'] == "章节撰写器":
            message="人物卡\n"+character_message+f"第{count}章细纲\n"+message+f"生成第{count}章的详细内容"
            message=chat_ollama_stream_post(message,item)
            break
